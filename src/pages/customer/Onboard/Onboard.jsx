...
import { FIELD_REQUIRED_MESSAGE, EMAIL_RULE, EMAIL_RULE_MESSAGE, PHONE_NUMBER_RULE, PHONE_NUMBER_RULE_MESSAGE, PASSWORD_RULE, PASSWORD_RULE_MESSAGE, PASSWORD_CONFIRMATION_MESSAGE } from '~/utils/validators'
import { toast } from 'react-toastify'
import PasswordField from '~/components/Form/PasswordField'
import PasswordStrengthIndicator from '~/components/Form/PasswordStrengthIndicator'
import { USER_ROLE } from '~/utils/constants'
import { useDispatch, useSelector } from 'react-redux'
import { selectCurrentCustomer, updateCustomerApi } from '~/redux/user/customerSlice'
...
function Onboard() {
  ...
  useEffect(() => {
    if (currentCustomer) {
      ...
      // Set role
      if (currentCustomer.role) {
        setRole(currentCustomer.role)
      }

      // Set address if exists
      if (currentCustomer.address) {
        setValue('street', currentCustomer.address.street || '')
        // We'll need to set province/ward based on address data
        // For now, we'll load provinces and try to match
      }
    }
  ...
  const validateCurrentStep = () => {
    ...
    if (step === 0) {
      ...
      // Validate Security Information - Password is required for onboarding
  ...
  const submitOnboard = async (data) => {
    const {
      companyName,
      firstName,
      lastName,
    ...
    toast.promise(dispatch(updateCustomerApi(updateData)), {
      ...
      if (!res.error) {
        ...
        if (role === USER_ROLE.PET_OWNER) {
          navigate('/profile')
        } else if (role === USER_ROLE.VET) {
          navigate('/vet-settings')
        } else if (role === USER_ROLE.SHELTER) {
    ...
    <Container maxWidth="lg" sx={{ px: { xs: 1, sm: 2, md: 3 } }}>
      ...
      <Box sx={{ my: 4, display: 'flex', justifyContent: 'center' }}>
        <Zoom in={true} style={{ transitionDelay: '200ms' }}>
          <MuiCard sx={{ minWidth: 440, maxWidth: 600, marginTop: '1em' }}>
            <Box sx={{
              margin: '1em',
              display: 'flex',
